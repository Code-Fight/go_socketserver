...
func HandelCMDConn(conn net.Conn,id <-chan []byte)  {
	//先发送登录和注册
	clientid:=<-id
	login:=[]byte{0x2a,0x00,0x00,0x00,0x0f,0x00,0x00,0xff,0xff,0x00,0x00,0x00,0x10,0x00,0x00,0x00,0x05,0x00,0x08,0x45,0x45,0x45}
	login[7] = clientid[0]
	login[8] = clientid[1]
	conn.Write(login)
...
func HandelRECVConn(conn net.Conn,idchan chan []byte)  {
	...
	//先发送登录和注册
	conn.Write([]byte{0x2a,0x00,0x00,0x00,0x0f,0x00,0x00,0xff,0xff,0x00,0x00,0x00,0x10,0x00,0x00,0x00,0x02,0x00,0x08,0x45,0x45,0x45})
